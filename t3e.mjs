...
// vary cols (page width) at single-page; narrower = taller but more legible glyphs after downsample
for(const cols of [312,360,408,468]){
  const imgs=await R.renderTextToPngsWithCharLimit(packed,cols,999999,{aa:true},4096);
  const billed=await imageBilledTokens(MODEL,imgs);
  const ib=[{type:'text',text:'Read data (↵=newline):'}];for(const im of imgs)ib.push({type:'image_url',image_url:{url:`data:image/png;base64,${b64(im.png)}`}});
  ...
  console.log(`cols=${cols} ${imgs.length}p [${imgs.map(i=>i.width+'x'+i.height).join(',')}] billed=${billed} density=${(C.length/billed).toFixed(1)} acc=[${accs.join(',')}]`);
}
