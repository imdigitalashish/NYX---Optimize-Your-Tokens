 ...
 *  narrow-baseline itself (rendered dumps, OCR, this very transcript). {@link neutralizeSentinel}
...
export interface RenderDensePagesOptions {
   ...
   *  that pre-reflow (the proxy's maybeReflow / history lockstep) pass false; `narrow-baseline export`
 ...
 * `renderTextToImages` (library.ts → `narrow-baseline export`) AND the proxy's `textToImageBlocks`
